...
// tiny-three-item and tiny-five-item are initialized by the Angular TinySliderDirective instead (avoids SSR hydration races).
if(false && document.getElementsByClassName('tiny-three-item').length > 0) {
    var slider = tns({
...
if(false && document.getElementsByClassName('tiny-five-item').length > 0) {
    var slider = tns({
...
try {
    ...
    const start2 = datepicker('.start2', { id: 2 })
    const end2 = datepicker('.end2', { id: 2 })

    const start3 = datepicker('.start3', { id: 3 })
    const end3 = datepicker('.end3', { id: 3 })
} catch (err) {
